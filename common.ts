...
import { readableStreamToString, ZapierActionsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ZapierActionsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ZapierActionsError(e, e.response?.data)
                }
                if (innerError instanceof ZapierActionsError) {
                    // Got "ZapierActionsError" from the above try block
                    throw innerError;
                }
